    ...
    serde_json::json!({
        ...
        "prompt_prefill_breakdown": {
            ...
            "forward_boundary": {
                ...
                "model_forward_classification": "TransformerModel::forward_with_workspace now records the model.forward.output owned-output boundary; the current behavior-preserving API still returns the final Candle Tensor without caller-provided output storage",
                ...
                "next_safe_change": "continue prompt_prefill.forward/model.forward allocation-layout work by adding or adopting caller-output-storage APIs before changing dense math or promoting packed Q8 sidecars",
...
fn warm_session_next_optimization_target(
    ...
    let (target, rationale, status) = match component {
        "prompt_prefill" => (
            ...
            "prompt prefill dominates aggregate allocation counters; prompt_prefill.forward is the measured subcomponent, and the next safe slice is explicit model.forward/output-storage API work before changing dense math",
            "model_forward_output_storage_blocked_by_owned_tensor_api",
        ),
        "prompt_prefill.forward" => (
            "model_forward_owned_output_boundary",
            "prompt_prefill.forward dominates aggregate allocation counters; TransformerForwardWorkspace now records model.forward.output and feed_forward.down_proj.output as owned-output boundaries, and reusable output storage remains blocked by the current behavior-preserving APIs",
            "model_forward_output_storage_blocked_by_owned_tensor_api",
        ),
        "prompt_prefill.embed" => (
