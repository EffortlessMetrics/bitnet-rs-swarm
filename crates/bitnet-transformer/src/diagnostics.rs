...
pub(crate) const QWEN_QPROJ_SOURCE_ORDER_Q8_ROW_MAPPING_PROOF_STAGE: &str =
    "attention.q_proj_source_order_q8_row_mapping_proof";
pub(crate) const QWEN_QPROJ_SOURCE_ORDER_Q8_ROW_MAPPING_PROOF_BOUNDARY: &str =
    "attention_q_proj_source_order_q8_row_mapping_proof";
...
pub(crate) struct QwenTraceSourceOrderQ8RowMappingProofEntry<'a> {
    pub output_index: usize,
    pub initial_bias: f32,
    pub source_order_output: f32,
    pub mapped_recomputed_output: f32,
    pub candle_recomputed_output: f32,
    pub eager_output: f32,
    pub abs_diff_mapped_vs_candle: f32,
    pub abs_diff_mapped_vs_eager: f32,
    pub terms_json: &'a str,
}

pub(crate) struct QwenTraceSourceOrderQ8RowMappingProof<'a> {
    pub stage: &'a str,
    pub layer_idx: usize,
    pub source_tensor: &'a str,
    pub gguf_tensor: &'a str,
    ...
    pub input_row: usize,
    pub q8_block_size: usize,
    pub entries: &'a [QwenTraceSourceOrderQ8RowMappingProofEntry<'a>],
}

pub(crate) fn qwen_trace_source_order_q8_row_mapping_proof(
    event: QwenTraceSourceOrderQ8RowMappingProof<'_>,
) {
    if !qwen_trace_active() || !qwen_trace_layer_enabled(event.layer_idx) {
        return;
    }
    ...
    let entries_json = event
        ...
        .map(|entry| {
            format!(
                "{{\"output_index\":{},\"initial_bias\":{},\"source_order_output\":{},\"mapped_recomputed_output\":{},\"candle_recomputed_output\":{},\"eager_output\":{},\"abs_diff_mapped_vs_candle\":{},\"abs_diff_mapped_vs_eager\":{},\"terms\":[{}]}}",
                entry.output_index,
                qwen_trace_number(f64::from(entry.initial_bias)),
                qwen_trace_number(f64::from(entry.source_order_output)),
                qwen_trace_number(f64::from(entry.mapped_recomputed_output)),
                qwen_trace_number(f64::from(entry.candle_recomputed_output)),
                qwen_trace_number(f64::from(entry.eager_output)),
                qwen_trace_number(f64::from(entry.abs_diff_mapped_vs_candle)),
                qwen_trace_number(f64::from(entry.abs_diff_mapped_vs_eager)),
                entry.terms_json
            )
        })
        .collect::<Vec<_>>()
    ...
    qwen_trace_write_line(&format!(
        "{{\"kind\":\"qwen_trace_source_order_q8_row_mapping_proof\",\"stage\":\"{}\",\"step\":{},\"layer\":{},\"source_tensor\":\"{}\",\"gguf_tensor\":\"{}\",\"boundary\":\"{}\",\"dense_hook_identity\":\"{}\",\"runtime_disabled\":true,\"default_runtime_preserved\":true,\"mapping_rule\":\"runtime_weight_idx=output_index*source_input_dim+input_index; source_order_weight_idx=input_index*source_output_dim+output_index\",\"source_input_dim\":{},\"source_output_dim\":{},\"input_row\":{},\"q8_block_size\":{},\"entries\":[{}]}}",
        qwen_trace_escape(event.stage),
        step,
        event.layer_idx,
        qwen_trace_escape(event.source_tensor),
...
mod tests {
    use super::{
        ...
        QWEN_QPROJ_SOURCE_ORDER_Q8_ROW_MAPPING_PROOF_STAGE, qwen_trace_f32_values_json,
        sha256_f32_le,
    };
    ...
    fn source_order_q8_candidate_boundary_constants_are_stable() {
        ...
        assert_eq!(
            QWEN_QPROJ_SOURCE_ORDER_Q8_ROW_MAPPING_PROOF_STAGE,
            "attention.q_proj_source_order_q8_row_mapping_proof"
        );
        assert_eq!(
            QWEN_QPROJ_SOURCE_ORDER_Q8_ROW_MAPPING_PROOF_BOUNDARY,
