...
Post-#1064 question matrix refresh: 2026-06-04
...
## POWER-006 Low-Power Question Matrix

This matrix routes the remaining `low_power` power-evidence questions to
current evidence, missing receipt shape, owner issues, forbidden claims, and
the next smallest PR. It is a review aid only; it does not run inference,
refresh committed hardware receipts, change route policy, promote `low_power`,
claim speedup, claim power advantage, prove measured temperatures, prove native
GPU/NPU acceleration, or prove BitNet QK256/I2_S behavior.

| Question | Current evidence | Missing receipt shape | Owner issue | Forbidden claims | Next smallest PR |
| --- | --- | --- | --- | --- | --- |
| Can the 258V start the POWER-006 route-sample window now? | Current committed and watch evidence still fails closed on AC/online state: `BatteryStatus=2`, `PowerOnline=True`, `Discharging=False`, `ac_power_inferred=true`, `battery_mode_sample_recorded=false`, and `requirement_satisfied=false`. | A strict before receipt from `telemetry-context --require-battery --strict` while actually on battery, with `battery_mode_sample_recorded=true`, `requirement_satisfied=true`, `ac_power_inferred=false`, and WMI battery/discharging corroboration when available. | [#1064](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1064) | No battery-mode, route-sample, power, or promotion claim from AC, charging, unknown, or one-point telemetry. | No PR until the physical battery preflight passes or exposes a missing/ambiguous receipt field. |
| What before/after telemetry is required? | The schema records Win32 battery percent, WMI capacity fields, AC/discharge state, power scheme, thermal availability, and strict capture requirements. AC-only before/after attempts are blocker evidence only. | Matched before and after battery telemetry receipts from the same sample window, both satisfying the strict predicate and preserving source fields, elapsed context, power scheme, thermal availability, and unavailable-field status. | [#1064](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1064) | No energy proxy or battery drain claim from one point, mixed AC/battery points, missing WMI corroboration when exposed, or coerced unavailable fields. | A narrow schema/checker PR only if the physical run cannot represent a required before/after field honestly. |
| Which `low_power` route samples are needed? | The runbook and tracker name CPU baseline, OpenVINO GPU candidate, and OpenVINO NPU candidate sample receipts, but no valid battery-mode route samples exist yet. | Three profile-scoped `low_power` ask receipts on battery for `dense_slm_default_cpu`, `dense_slm_openvino_gpu_candidate`, and `dense_slm_openvino_npu_candidate`, each with answer gate, selected backend/runtime/device, model/export identity, timing, battery context, and `fallback_used=false`. | [#1064](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1064) | No route promotion, GPU/NPU power advantage, native accelerator, or low_power readiness claim from planning receipts or AC-linked ask telemetry context. | Wait for the battery runbook sequence; open a route-sample receipt PR only after strict battery preflight passes. |
| What makes the energy proxy valid? | The current energy-proxy support can record AC-only attempts, but those attempts are explicitly invalid for POWER-006 evidence. | `lunar-lake-low-power-energy-proxy.json` built from before/after battery-mode telemetry receipts for the sampled route/profile window, with sample count, route/profile identity, capacity or percent delta source, elapsed context, and invalid-source gaps preserved. | [#1064](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1064) | No benchmark-qualified power advantage, watts, energy, or promotion claim from AC-only proxy attempts, latency-only receipts, percent noise, or missing before/after battery evidence. | A narrow builder/status PR only if the physical battery receipts cannot be converted into a valid proxy without widening claims. |
| How should thermal evidence be handled? | Thermal zones are visible on this host, but measured temperature values are unavailable or zero in current receipts. That is availability context, not measured-temperature evidence. | Route-window telemetry must carry thermal availability and explicitly mark measured temperatures unavailable when they are unavailable; measured-temperature claims require real nonzero measured readings tied to the sample window. | [#1064](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1064) | No measured-temperature, thermal advantage, or thermal stability claim from zone visibility, zero values, or unavailable readings. | A narrow thermal-status PR only if the battery run exposes ambiguity between unavailable and measured thermal fields. |
| Can OpenVINO device properties prove power behavior? | OpenVINO properties can identify CPU/GPU/NPU devices, runtime/cache state, and performance hints; the local property set does not provide direct route-level watts, battery drain, or temperature readings. | Any future OpenVINO-derived power claim would need a direct, stable property or external measurement tied to route/profile samples and corroborated by battery telemetry. | [#1064](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1064) | No OpenVINO GPU/NPU power, wattage, temperature, native acceleration, or low_power promotion claim from device identity or cache/performance properties alone. | None from current OpenVINO properties; keep power evidence in the battery telemetry/proxy lane. |
| When can `low_power` route policy change? | Current route policy keeps `low_power` unpromoted. Existing ask, schema, plan, and AC-blocked harness receipts are operator readiness and fail-closed evidence only. | Refreshed power-profile, regression-v2, operator-comparison, excellence-audit, and goal-checklist artifacts built from valid battery-mode before/after telemetry, fallback-free CPU/GPU/NPU route samples, a valid energy proxy, and benchmark-qualified power advantage. | [#1064](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1064), [#1245](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1245) | No route-policy mutation, low_power promotion, speedup, power-advantage, broad quality, or native accelerator claim before the full evidence bundle exists. | Use #1245 only after #1064 produces accepted evidence or a concrete contradiction requiring a route-policy review. |
| Does POWER-006 prove BitNet or native GPU/NPU behavior? | POWER-006 is a dense SLM Lunar Lake power-profile evidence item. It is separate from native OpenCL/NPU kernels and separate from packed BitNet QK256/I2_S behavior. | Separate native accelerator or BitNet semantic proof with model/kernel identity, CPU reference, fallback false, answer gates, and exact route/profile timing. | Future native accelerator or BitNet semantic issue only | No dense-SLM-as-BitNet, full BitNet accelerator inference, packed QK256/I2_S, native OpenCL, or native NPU claim from POWER-006. | No PR in this power telemetry lane; keep native/BitNet accelerator proof in separate lanes. |
