...
| SLM-CPU-089 | ready | Next residual-add output-storage gate after SLM-CPU-088; it must either add or precisely specify a behavior-preserving reusable-storage API for `transformer.block.output`, or emit a blocker proving Candle still forces owned Tensor output. No runtime improvement, speedup, packed-Q8 promotion, Q4/Q5, accelerator, server, Qwen3.5, or BitNet QK256 claim is allowed without before/after behavior receipts. |
