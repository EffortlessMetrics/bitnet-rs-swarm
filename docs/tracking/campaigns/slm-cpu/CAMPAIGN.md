...
| SLM-CPU-086 | ready | Next Kaby performance slice: prove whether `model.final_norm.output` can use a behavior-preserving caller-output-storage helper, or record the exact remaining LayerNorm/RMSNorm API blocker before moving to residual-add or dense-math work. |
