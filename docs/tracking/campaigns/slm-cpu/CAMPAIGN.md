...
| SLM-CPU-087 | ready | Queue the next Kaby Lake Qwen3 Q8_0 allocation-layout slice at the residual-add / `transformer.block.output` boundary after SLM-CPU-086; implementation must preserve the appliance behavior oracle and cannot claim speedup, packed-Q8 promotion, Q4/Q5, accelerators, server, Qwen3.5, or BitNet QK256 changes without before/after receipts. |
