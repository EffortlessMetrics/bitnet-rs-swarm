...
| SLM-CPU-087 | merged | #437 queued the next Kaby Lake Qwen3 Q8_0 allocation-layout slice at the residual-add / `transformer.block.output` boundary after SLM-CPU-086; implementation must preserve the appliance behavior oracle and cannot claim speedup, packed-Q8 promotion, Q4/Q5, accelerators, server, Qwen3.5, or BitNet QK256 changes without before/after receipts. |
| SLM-CPU-088 | merged | #459 recorded the residual-add / `transformer.block.output` allocation-layout boundary with residual input and feed-forward branch output shape evidence; it keeps the Qwen3 Q8_0 appliance oracle as the gate and makes no speedup, packed-Q8 promotion, Q4/Q5, accelerator, server, Qwen3.5, or BitNet QK256 claim. |
| SLM-CPU-089 | merged | #471 queued the next residual-add output-storage gate after SLM-CPU-088 and preserved the no-runtime-optimization, no-speedup, no-Q4/Q5, no-accelerator, no-server, no-Qwen3.5, and no-BitNet-QK256 claim boundary. |
| SLM-CPU-090 | merged | #483 recorded the exact Candle residual-add output-storage blocker for `transformer.block.output` and exposed it through the warm-session allocation audit JSON; it does not claim a residual-add runtime optimization, allocation improvement, speedup, sustained throughput, Q4/Q5 support, accelerator/server support, Qwen3.5, or BitNet QK256 changes. |
| SLM-CPU-091 | merged | #507 queued the next non-duplicative performance target after the residual-add storage path was blocked at Candle public Tensor APIs; it moved the lane back to the `model.logits` / output-head tensor allocation boundary identified by the Kaby dashboard. |
...
| SLM-CPU-106 | merged | #681 defined the fail-closed q_norm-input receipt comparator contract while keeping proof blocked on the runtime hook, Qwen3/Qwen2.5 before-after receipts, q_norm-input tensor identity, and accumulator-order evidence. |
| SLM-CPU-107 | ready | Add or precisely block the runtime-disabled q_norm-input hook and receipt tensor-identity surface needed before the selected materialization boundary can be behavior-proven. |
