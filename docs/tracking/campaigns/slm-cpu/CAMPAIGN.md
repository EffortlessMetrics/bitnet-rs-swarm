...
| SLM-CPU-085 | in_progress | Further narrows the final-norm/layer-output caller-output-storage boundary after #399 by recording machine-checkable `model.final_norm.output` and `transformer.block.output` blockers while preserving the Qwen3 Q8_0 appliance behavior oracle before any bounded allocation improvement can be claimed. |
