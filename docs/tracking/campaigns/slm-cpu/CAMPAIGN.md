...
| SLM-CPU-091 | ready | Next non-duplicative performance target after the residual-add storage path was blocked at Candle public Tensor APIs; it moves the lane back to the `model.logits` / output-head tensor allocation boundary identified by the Kaby dashboard. |
