...
| SLM-CPU-090 | ready | Next residual-add output-storage implementation/blocker slice: either add a behavior-preserving reusable-storage helper for `transformer.block.output`, or emit a machine-checkable blocker naming the exact Candle Tensor API limitation. Any runtime change remains gated by Qwen3 Q8_0 before/after behavior receipts. |
