...
| SLM-CPU-083 | ready | Next safe Kaby performance slice: return to the `prompt_prefill.forward` / model-forward allocation path after the packed-Q8 sidecar repeated timing regression, preserving the Qwen3 Q8_0 behavior oracle and making no speedup or default-runtime claim without before/after receipts. |
