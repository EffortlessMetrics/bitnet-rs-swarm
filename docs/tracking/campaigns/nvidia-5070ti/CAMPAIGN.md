...
| Qwen3 0.6B | product CLI ready for bounded ask/chat user paths after artifact, CPU sanity, all-layer plan, one-token, short-decode, warm-session, benchmark-review, ask user-path, chat user-path, and product-promotion review evidence; exact-profile server-ready only for the current-source non-streaming shared-engine `/v1/chat/completions` RTX 5070 Ti dense CUDA receipt accepted by CUDA-MODEL-014B; speed, benchmark-qualified speed, full-residency, broad dense GGUF, Qwen2.5 inheritance, and BitNet QK256 claims remain false | `ci/hardware/windows-9950x3d-rtx5070ti/2026-05-19/server-strict-dense-qwen3-q8-smoke.json` | repeated exact-profile performance comparator source receipts before speed, benchmark-qualified, or full-residency review |
...
| CUDA-DENSE-QWEN25-OPS-004 | merged | PR #6076 hardened the Qwen2.5 logits-transfer reduction contract so `device_to_host_bytes_reduced=true` requires a CUDA device-side top-k or greedy sampler identity, `sampling_location=cuda_device`, measured D2H below the full-logits envelope, preserved selected-token/top-k/quality evidence, and correct byte math. Follow-on PR #6098 added the device top-k proof-receipt path that consumes this contract, while preserving false speedup, benchmark-qualified speed, full residency, broad dense GGUF, server-promotion, and BitNet QK256 claims. |
