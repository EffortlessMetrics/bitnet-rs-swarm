...
Current focus: `M3MBA-025` aligns accepted M3 artifact metadata with a separate
M4 strict-proof checklist without manufacturing M4 proof. `M3MBA-023` is
...
`M3MBA-025` is in progress. M4-alignment work must remain a checklist and must
require fresh M4 receipts before any M4 proof claim. Any later performance work
remains scoped to the exact M3 Air host and proof-lane labels represented by
