...
Current focus: `M3MBA-024` records authority/storage preflight for the blocked
0.7B and 3B BitNet candidates before any new large download. `M3MBA-023` is
merged in #1526 and exposed the bounded performance profile, but performance
...
`M3MBA-024` is in progress. Secondary BitNet artifact work must stay in
preflight/reporting paths and must not start a new large local download. Any
later performance work remains scoped to the exact M3 Air host and proof-lane
labels represented by the device/profile contract.
