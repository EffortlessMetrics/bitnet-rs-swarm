...
Current focus: `M3MBA-024` is merged in #1534 and keeps the blocked 0.7B and 3B
BitNet candidates in authority/storage preflight before any new large download.
The next proposed slice is `M3MBA-025`, which aligns accepted M3 artifact
metadata with a separate M4 strict-proof checklist without manufacturing M4
proof. `M3MBA-023` is merged in #1526 and exposed the bounded performance
profile, but performance timing still requires future committed receipts before
any speed claim. The Microsoft 2B I2_S artifact is accepted only for the
...
expose the GGUF artifacts required by their command shapes. Accuracy,
performance, artifact, and handoff-alignment work now use the structured M3 Air
...
`M3MBA-024` is merged. The next implementation slice is `M3MBA-025`;
M4-alignment work must remain a checklist and must require fresh M4 receipts
before any M4 proof claim. Any later performance work remains scoped to the
exact M3 Air host and proof-lane labels represented by the device/profile
contract.
