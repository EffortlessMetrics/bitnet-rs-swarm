...
Current focus: `M3MBA-023` builds on the merged `M3MBA-022` accuracy comparison
profile by exposing the existing release-mode M3 Air dense SLM performance
profile through the staged hardware workflow. The workflow records token budget,
host power/thermal/storage context, phase artifacts, and timeout-cap provenance
from completed healthy runs plus cushion before selected performance timing can
be used. The Microsoft 2B I2_S artifact is accepted only for the recorded M3 Air
BitNet.cpp reference-runner context and is ready to seed separate M4 strict-proof
work with fresh M4 receipts. `M3MBA-006` and
...
`M3MBA-022` is merged. The current implementation slice is `M3MBA-023`;
performance work should remain scoped to the exact M3 Air host and
