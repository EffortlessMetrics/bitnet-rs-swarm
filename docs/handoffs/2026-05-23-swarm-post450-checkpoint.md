...
- #450 `docs(slm-cpu): queue SLM-CPU-088`
  ...
  - Claim boundary: no runtime code, dense math, packed-Q8 sidecar promotion,
    speedup, sustained throughput, Q4/Q5 runtime support, server inference,
    GPU/NPU/OpenVINO/UHD 620 execution, Qwen3.5 support, or BitNet QK256/I2_S
...
- #445 `docs(slm-cpu): close SLM-CPU-087 queue slice`
  - Closed as superseded by #444. It duplicated the SLM-CPU-087 closeout and
    widened scope by seeding `SLM-CPU-088`; the SLM-CPU-088 queueing was
    handled separately by #450.
