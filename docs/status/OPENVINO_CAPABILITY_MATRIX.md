...
| CPU slow-path and OpenVINO CPU decision boundary | `docs/research/lunar-lake-cpu-slow-path.md`, `docs/reviews/lunar-lake-cpu-route-decision.md`, [#1232](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1232); receipt-write and telemetry timing scope under [#1291](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1291) |
...
| Row | Next proof |
| --- | --- |
| Qwen2.5 OpenVINO CPU | #1281/#1290 closed the resident prompt-render, quality-gate, detokenize, and memory lifecycle field gap. Use #1291 to define whether `receipt_write_ms` and `telemetry_ms` are profile metrics, aggregate/session context, or accepted explicit-unavailable fields before refreshing #1280 physical resident artifacts. Keep #1232 / #1280 evidence diagnostic until the strict resident summary is reviewable, and do not backfill profile timing from aggregate probes. No OpenVINO CPU promotion or speedup claim is valid while model-format, timing-scope, tokenization, and runtime-scope mismatches remain unresolved or unaccepted. |
