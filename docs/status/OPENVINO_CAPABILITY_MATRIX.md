...
| CPU slow-path and OpenVINO CPU decision boundary | `docs/research/lunar-lake-cpu-slow-path.md`, `docs/reviews/lunar-lake-cpu-route-decision.md`, [#1232](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1232); receipt-write and telemetry timing scope closed by [#1291](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1291) / [#1292](https://github.com/EffortlessMetrics/bitnet-rs-swarm/pull/1292) |
...
| Qwen2.5 OpenVINO NPU | Keep `warm_resident` scoped promotion only. Cold/cache work stays under #1119; #1282/#1286 records that OpenVINO cache docs support configuration/provenance fields, but current GenAI receipts still lack direct runtime cache-hit truth. Future cache work needs a documented cache-hit property, parseable runtime log, accepted stricter proxy policy, or profile-matched phase evidence for the exact mode. #1242/#1248 closed the AUTO debug-log parser helper; future source wiring must stay under #1149 or a new narrow child issue. No cold one-off, `ask_short`, `ask_normal`, or `low_power` expansion follows from cache timing, cache configuration support, or AUTO debug-log evidence alone. |
