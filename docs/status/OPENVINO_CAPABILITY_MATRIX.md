...
| Row | Next proof |
| --- | --- |
| Qwen2.5 OpenVINO CPU | #1281/#1290 closed the resident prompt-render, quality-gate, detokenize, and memory lifecycle field gap. #1291/#1292 closed the receipt-write and telemetry scope contract: profile `receipt_write_ms` and `telemetry_ms` stay explicit `not_exposed` fields in the current resident-session scope and must not be backfilled from aggregate/session probes. #1311/#1319 closed the reviewable-versus-qualified status contract: `diagnostic_package_reviewable=true` may be recorded when only those fields remain unavailable, while `resident_phase_qualified=false` and `benchmark_qualified=false` stay strict. #1280/#1334 closed the physical resident package with 33 prompts, 32 warm asks after first, `fallback_used=false`, one model/tokenizer load, passing quality, and deterministic text/token IDs. #1232 remains open for an issue-shaped next step: scoped aggregate receipt-write/telemetry contract, matched Rust GGUF versus OpenVINO CPU comparison plan, or topology/affinity/utilization/frequency/thermal evidence if available. No OpenVINO CPU promotion or speedup claim is valid while model-format, timing-scope, tokenization, and runtime-scope blockers remain unresolved or unaccepted. |
...
| BitNet semantic intake | Use #1178 for true shared BitNet semantic freshness and targeted CPU reference rerun decisions. The #1263/#1267 diagnostic-only pattern is closed and only covers reviewed non-trigger classification such as #1257/#1264; current #1178 audit through `d6197833b299ea3b5d547f32f72cc8eef2ed88bc` keeps `rerun_required=false` and `intake_ready=true`, with post-`c8076ea` A770-071 #1305/#1306 work classified as docs/receipts/tracking-only. Ambiguous shared-surface scope must not be treated as current BitNet evidence. |
