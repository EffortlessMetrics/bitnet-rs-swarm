...
| CPU slow-path and OpenVINO CPU decision boundary | `docs/research/lunar-lake-cpu-slow-path.md`, `docs/reviews/lunar-lake-cpu-route-decision.md`, [#1232](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1232); matched Rust GGUF CPU versus OpenVINO CPU comparison owned by [#1365](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1365); CPU topology/affinity evidence owned by [#1370](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1370); resident aggregate/session overhead scope owned by [#1374](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1374); physical resident package closed by [#1280](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1280) / [#1334](https://github.com/EffortlessMetrics/bitnet-rs-swarm/pull/1334); diagnostic reviewable versus qualified status closed by [#1311](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1311) / [#1319](https://github.com/EffortlessMetrics/bitnet-rs-swarm/pull/1319); receipt-write and telemetry timing scope closed by [#1291](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1291) / [#1292](https://github.com/EffortlessMetrics/bitnet-rs-swarm/pull/1292) |
| GPU profile promotion review boundary | `docs/reviews/lunar-lake-openvino-gpu-promotion-review.md`, `docs/reviews/lunar-lake-openvino-gpu-phase-profile-review.md`; `ask_short` / `ask_normal` receipt-invalidation guard tracked by [#1373](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1373); phase-boundary hardening closed by [#1241](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1241) / [#1268](https://github.com/EffortlessMetrics/bitnet-rs-swarm/pull/1268); future route-policy decisions use [#1245](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1245) |
| NPU cold/cache and warm-resident boundary | `docs/research/lunar-lake-npu-cold-start.md`, `docs/reviews/lunar-lake-npu-cold-cache-evidence.md`, `docs/reviews/lunar-lake-npu-warm-resident-acceptance.md`, [#1119](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1119); direct cache-hit truth child tracked by [#1371](https://github.com/EffortlessMetrics/bitnet-rs-swarm/issues/1371); cache source-boundary audit recorded by [#1282](https://github.com/EffortlessMetrics/bitnet-rs-swarm/pull/1282) / [#1286](https://github.com/EffortlessMetrics/bitnet-rs-swarm/pull/1286) |
...
| Row | Next proof |
| --- | --- |
| Qwen2.5 OpenVINO CPU | #1281/#1290 closed the resident prompt-render, quality-gate, detokenize, and memory lifecycle field gap. #1291/#1292 closed the receipt-write and telemetry scope contract: profile `receipt_write_ms` and `telemetry_ms` stay explicit `not_exposed` fields in the current resident-session scope and must not be backfilled from aggregate/session probes. #1374 owns any future aggregate/session overhead fields and keeps them diagnostic unless a later accepted contract changes qualification. #1311/#1319 closed the reviewable-versus-qualified status contract: `diagnostic_package_reviewable=true` may be recorded when only those fields remain unavailable, while `resident_phase_qualified=false` and `benchmark_qualified=false` stay strict. #1280/#1334 closed the physical resident package with 33 prompts, 32 warm asks after first, `fallback_used=false`, one model/tokenizer load, passing quality, and deterministic text/token IDs. #1232 remains open for resident Rust GGUF phase evidence, while #1365 owns the matched Rust GGUF versus OpenVINO CPU comparison plan. No OpenVINO CPU promotion or speedup claim is valid while model-format, timing-scope, tokenization, and runtime-scope blockers remain unresolved or unaccepted. |
| Qwen2.5 OpenVINO GPU | Keep current profile-scoped promotion. #1373 owns the `ask_short` / `ask_normal` receipt-invalidation guard: those promotions stay valid only while route identity, fallback-free evidence, corpus quality, direct-token visibility, profile timing, benchmark qualification, selected-device evidence, and negative claim-boundary fields remain present and uncontradicted. #1241/#1268 added the current `phase_claim_boundary` hardening for `prefill_heavy` and `decode_heavy`; future phase-split receipts or route-policy changes need a new narrow evidence issue or #1245 review finding. No broader GPU promotion or power claim follows without a fresh route review. `low_power` remains blocked by #1064. |
| Qwen2.5 OpenVINO NPU | Keep `warm_resident` scoped promotion only. Cold/cache work stays under #1119, with #1371 owning direct cache-hit truth: future cache work needs a documented cache-hit property, parseable runtime log, explicit unavailable truth-source handling, accepted stricter proxy policy, or profile-matched phase evidence for the exact mode. #1282/#1286 records that OpenVINO cache docs support configuration/provenance fields, but current GenAI receipts still lack direct runtime cache-hit truth. #1242/#1248 closed the AUTO debug-log parser helper, #1251/#1252 closed the repeatable capture wrapper, and #1254 preserved SDPA warning / AUTO fallback-disabled line refs without changing the phase receipt `fallback_used=false` decision. Future AUTO work must stay under #1149 or a new narrow child issue only for materially useful wrapper-generated evidence, a concrete selected-device API/bridge, or route review after all gates. No cold one-off, `ask_short`, `ask_normal`, or `low_power` expansion follows from cache timing, cache configuration support, or AUTO debug-log evidence alone. |
