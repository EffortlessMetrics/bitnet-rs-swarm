...
| BitNet | ask | `batch_only` | explicit `bitnet mac ask --model-id microsoft-bitnet-b1.58-2B-4T-i2s ...` | `M4-BITNET-EX-003` / `bitnet_benchmark` (`bitnet_apple_m4_benchmark_v1`); `M4-BITNET-EX-005` / `bitnet_one_shot_failure` (`bitnet_apple_m4_mac_ask_failure`) | Accepted artifact/tokenizer one-shot only; not chat, serve, broad quality, or speed proof. |
