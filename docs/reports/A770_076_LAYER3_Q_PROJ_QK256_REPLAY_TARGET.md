# A770-076 Layer 3 Q Projection QK256 Replay Target

## Scope

A770-076 advances the focused QK256 replay packet by converting the next
remaining target, layer-3 `q_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:3:q_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer3-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer3-q-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer3-q-proj-qk256-replay-target/a770-opencl-qk256-layer3-q-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the A770-068 through
A770-075 overlays, and adds one supplemental focused source for the new layer-3
`q_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer3-q-proj-qk256-replay-target/a770-opencl-qk256-layer3-q-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 10
runnable_target_count = 10
```
...
The selected-device production replay executed all ten targets on Intel Arc
A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 10
blocked_target_count = 80
failed_target_count = 0
matched_selected_device_bits_count = 10
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 20
host_to_device_bytes = 32000
device_to_host_bytes = 560
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 80
```

## Interpretation

A770-076 starts the layer-3 Q/K/V burn-down by adding `q_proj` after A770-075
completed the layer-2 Q/K/V trio. This is useful correctness evidence, not
...
`dispatch_replay_missing` set. The direct continuation is layer-3 `k_proj`
