# A770-104 Layer 12 K Projection QK256 Replay Target

## Scope

A770-104 advances the focused QK256 replay packet by converting one more
remaining target, layer-12 `k_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:12:k_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer12-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer12-k-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer12-k-proj-qk256-replay-target/a770-opencl-qk256-layer12-k-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the previous
thirty-seven focused replay sources, and adds one supplemental focused source
for the new layer-12 `k_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer12-k-proj-qk256-replay-target/a770-opencl-qk256-layer12-k-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 38
runnable_target_count = 38
```

The runnable targets are the layer-0 through layer-11 Q/K/V trios plus layer-12
`q_proj` and `k_proj`.

The selected-device production replay executed all thirty-eight targets on
Intel Arc A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 38
blocked_target_count = 52
failed_target_count = 0
matched_selected_device_bits_count = 38
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 76
host_to_device_bytes = 121600
device_to_host_bytes = 2128
```
...
target_id = a770_summary_seed770024_keywords_014:step9:12:k_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
```text
dispatch_replay_missing = 52
first_blocked_target = a770_summary_seed770024_keywords_014:step9:12:v_proj
```

## Interpretation

A770-104 continues the layer-12 focused replay Q/K/V trio by adding `k_proj`
after A770-103 added layer-12 `q_proj`. This is useful correctness evidence,
...
`dispatch_replay_missing` set. The direct continuation is layer-12 `v_proj`
...
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer12-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer12-k-proj-raw-operands.json --per-prompt-timeout-seconds 300
target/debug/a770-opencl-production-replay-instrumentation.exe --focused-source ... --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer12-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer12-k-proj-raw-operands.json --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-104 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer12-k-proj-qk256-replay-target/a770-opencl-qk256-layer12-k-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer12-k-proj-qk256-replay-target/a770-opencl-qk256-layer12-k-proj-target-replay.json
pwsh receipt assertion for target_count=90, dispatch_replay_target_count=38, runnable_target_count=38, executed_target_count=38, blocked_target_count=52, failed_target_count=0, matched_selected_device_bits_count=38, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-104, and layer-12 k_proj executed fallback-free with selected-device bit matches
```
...
BITNET_QKV_PROJECTION_DISPATCH_REPLAY_LAYER=12
