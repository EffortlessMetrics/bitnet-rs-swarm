# A770-072 Layer 1 V Projection QK256 Replay Target

## Scope

A770-072 advances the focused QK256 replay packet by converting the next
remaining layer-1 target, `v_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:1:v_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer1-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer1-v-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer1-v-proj-qk256-replay-target/a770-opencl-qk256-layer1-v-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the A770-068 through
A770-071 overlays, and adds one supplemental focused source for the new layer-1
`v_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer1-v-proj-qk256-replay-target/a770-opencl-qk256-layer1-v-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 6
runnable_target_count = 6
```
...
The selected-device production replay executed all six targets on Intel Arc
A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 6
blocked_target_count = 84
failed_target_count = 0
matched_selected_device_bits_count = 6
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 12
host_to_device_bytes = 19200
device_to_host_bytes = 336
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 84
```

## Interpretation

A770-072 completes the layer-1 Q/K/V trio by adding `v_proj` after the
A770-070 layer-1 `q_proj` and A770-071 layer-1 `k_proj` replay targets. This
...
`dispatch_replay_missing` set. The direct continuation is layer-2 `q_proj`
