# A770-150 Layer 27 V Projection QK256 Replay Target

## Scope

A770-150 advances the focused QK256 replay packet by converting one more
remaining target, layer-27 `v_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:27:v_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer27-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer27-v-proj-raw-operands.json
```

The focused answer-corpus run receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer27-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer27-v-proj-raw-operands-runs/a770_summary_seed770024_keywords_014.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer27-v-proj-qk256-replay-target/a770-opencl-qk256-layer27-v-proj-target-replay-manifest.json
```
...
previous A770-149 focused source list.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer27-v-proj-qk256-replay-target/a770-opencl-qk256-layer27-v-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 84
runnable_target_count = 84
```

The runnable targets are the layer-0 through layer-27 Q/K/V trios.

The selected-device production replay executed all eighty-four runnable targets
on Intel Arc A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 84
blocked_target_count = 6
failed_target_count = 0
matched_selected_device_bits_count = 84
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 168
host_to_device_bytes = 268800
device_to_host_bytes = 4704
```
...
target_id = a770_summary_seed770024_keywords_014:step9:27:v_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
focused_device_output_bits = 1072984280
...
```text
dispatch_replay_missing = 6
first_blocked_target = a770_summary_seed770024_keywords_014:step9:28:q_proj
first_blocked_reason = dispatch_replay_missing
```

## Interpretation

A770-150 completes the layer-27 Q/K/V trio under the same selected-device,
...
`dispatch_replay_missing` set. The direct continuation is layer-28 `q_proj`
...
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer27-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer27-v-proj-raw-operands.json --per-prompt-timeout-seconds 300
target/debug/a770-opencl-production-replay-instrumentation.exe --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer27-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer27-v-proj-raw-operands.json --focused-source ... --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-150 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer27-v-proj-qk256-replay-target/a770-opencl-qk256-layer27-v-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer27-v-proj-qk256-replay-target/a770-opencl-qk256-layer27-v-proj-target-replay.json
PowerShell receipt assertion for target_count=90, dispatch_replay_target_count=84, runnable_target_count=84, executed_target_count=84, blocked_target_count=6, failed_target_count=0, matched_selected_device_bits_count=84, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-150, exactly one additional executed target beyond A770-149, and layer-27 v_proj executed fallback-free with selected-device bit matches
...
npx markdownlint-cli2 --config .markdownlint.jsonc docs/reports/A770_150_LAYER27_V_PROJ_QK256_REPLAY_TARGET.md docs/reports/A770_FAST_INFERENCE_FRONTIER.md docs/tracking/campaigns/intel-a770/CAMPAIGN.md
git diff --check
...
campaign check intel-a770` path was not used as A770-150 merge evidence. The
same native Windows route timed out in the immediately prior A770-148 packet
while compiling `sentencepiece-sys v0.13.1`; the WSL-owned `xtask` campaign
check and generator check above are the substitute campaign evidence for this
docs, tracker, generated-dashboard, and receipt-only packet.
