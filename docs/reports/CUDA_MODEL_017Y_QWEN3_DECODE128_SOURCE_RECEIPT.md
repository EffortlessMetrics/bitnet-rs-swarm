# CUDA-MODEL-017Y Qwen3 Decode-128 Source Receipt

Status: `decode_128_from_warm_context` source receipt 1 / 3; aggregate still blocked
...
This report records the first current-source `decode_128_from_warm_context`
...
still has only one of the required three source receipts.
...
```text
run-01 total_ms=776068.3361 first_token_ms=1247.4901 decode_total_ms=184953.8180 kernel_time_ms=748909.8247 kernel_launches=50432 H2D_bytes=639446688 D2H_bytes=77791232
```
...
decode_128_from_warm_context: 1 / 3
```

The next source-capture work should capture run-02 and run-03
`decode_128_from_warm_context` receipts with the same artifact, route, fallback,
and claim boundaries.

## Claim Boundary

This report proves only that CUDA-MODEL-017 has one current-source Qwen3
`decode_128_from_warm_context` strict CUDA source receipt.
...
- Any claim that one decode-128 source receipt satisfies the remaining profile
  receipts
