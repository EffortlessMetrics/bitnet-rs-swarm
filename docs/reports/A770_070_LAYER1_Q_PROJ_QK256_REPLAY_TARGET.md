# A770-070 Layer 1 Q Projection QK256 Replay Target

## Scope

A770-070 advances the focused QK256 replay packet by converting the first
remaining layer-1 target, `q_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:1:q_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer1-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer1-q-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer1-q-proj-qk256-replay-target/a770-opencl-qk256-layer1-q-proj-target-replay-manifest.json
```

It preserves the A770-067 90-target manifest shape, keeps the A770-068 and
A770-069 overlays, and adds one supplemental focused source for the new
layer-1 `q_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer1-q-proj-qk256-replay-target/a770-opencl-qk256-layer1-q-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 4
runnable_target_count = 4
```
...
The selected-device production replay executed all four targets on Intel Arc
A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 4
blocked_target_count = 86
failed_target_count = 0
matched_selected_device_bits_count = 4
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 8
host_to_device_bytes = 12800
device_to_host_bytes = 224
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 86
```

## Interpretation

A770-070 starts the layer-1 Q/K/V burn-down after the layer-0 Q/K/V trio was
made runnable by A770-069. This is useful correctness evidence, not speed
...
`dispatch_replay_missing` set. The direct continuation is layer-1 `k_proj`
