# A770-119 Layer 17 K Projection QK256 Replay Target

## Scope

A770-119 advances the focused QK256 replay packet by converting one more
remaining target, layer-17 `k_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:17:k_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer17-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer17-k-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer17-k-proj-qk256-replay-target/a770-opencl-qk256-layer17-k-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the previous
fifty-two focused replay sources, and adds one supplemental focused source for
the new layer-17 `k_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer17-k-proj-qk256-replay-target/a770-opencl-qk256-layer17-k-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 53
runnable_target_count = 53
```

The runnable targets are the layer-0 through layer-16 Q/K/V trios plus
layer-17 `q_proj` and `k_proj`.

The selected-device production replay executed all fifty-three runnable targets
on Intel Arc A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 53
blocked_target_count = 37
failed_target_count = 0
matched_selected_device_bits_count = 53
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 106
host_to_device_bytes = 169600
device_to_host_bytes = 2968
```
...
target_id = a770_summary_seed770024_keywords_014:step9:17:k_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
focused_device_output_bits = 1068635790
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 37
first_blocked_target = a770_summary_seed770024_keywords_014:step9:17:v_proj
first_blocked_reason = dispatch_replay_missing
```

## Interpretation

A770-119 continues the layer-17 focused replay Q/K/V trio by adding `k_proj`
after A770-118 added `q_proj`. This is useful correctness evidence, not speed
...
`dispatch_replay_missing` set. The direct continuation is layer-17 `v_proj`
...
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer17-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer17-k-proj-raw-operands.json --per-prompt-timeout-seconds 300
target/debug/a770-opencl-production-replay-instrumentation.exe --focused-source ... --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer17-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer17-k-proj-raw-operands.json --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-119 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer17-k-proj-qk256-replay-target/a770-opencl-qk256-layer17-k-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer17-k-proj-qk256-replay-target/a770-opencl-qk256-layer17-k-proj-target-replay.json
python receipt assertion for target_count=90, dispatch_replay_target_count=53, runnable_target_count=53, executed_target_count=53, blocked_target_count=37, failed_target_count=0, matched_selected_device_bits_count=53, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-119, and layer-17 k_proj executed fallback-free with selected-device bit matches
```
