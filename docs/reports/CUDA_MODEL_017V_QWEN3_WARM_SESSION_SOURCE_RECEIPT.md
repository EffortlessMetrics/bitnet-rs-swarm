# CUDA-MODEL-017V Qwen3 Warm Session Source Receipt

Status: `warm_session_3_turns` source receipt 1 / 3; aggregate still blocked
...
This report records the first current-source `warm_session_3_turns` strict CUDA
source receipt on the Windows 9950X3D + RTX 5070 Ti lane. It starts the warm
...
```text
run-01 total_ms=577244.4376 first_token_ms=979.9487 prefill_ms=540823.4681 decode_total_ms=24324.4043 kernel_time_ms=562670.181 kernel_launches=9456 H2D_bytes=639446688 D2H_bytes=14585856
```
...
CUDA-MODEL-017 acceptance still requires two additional `warm_session_3_turns`
source receipts before the warm-session source set is complete. The focused
`bitnet-bench-receipts` Qwen3 tests, focused `bitnet-cli` receipt-explain tests,
and `git diff --check` passed.

Local `campaign check nvidia-5070ti` and `check-model-coverage` commands timed
out after 15 minutes and 10 minutes respectively; those checks are expected to
run through the remote campaign/model gates for the PR.
...
The next source-capture work should capture the second
...
`warm_session_3_turns` strict CUDA source receipt.
...
- Any claim that this first warm-session receipt satisfies the remaining
  profile receipts
