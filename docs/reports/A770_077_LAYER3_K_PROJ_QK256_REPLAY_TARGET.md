# A770-077 Layer 3 K Projection QK256 Replay Target

## Scope

A770-077 advances the focused QK256 replay packet by converting the next
remaining target, layer-3 `k_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:3:k_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer3-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer3-k-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer3-k-proj-qk256-replay-target/a770-opencl-qk256-layer3-k-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the A770-068 through
A770-076 overlays, and adds one supplemental focused source for the new layer-3
`k_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer3-k-proj-qk256-replay-target/a770-opencl-qk256-layer3-k-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 11
runnable_target_count = 11
```
...
The selected-device production replay executed all eleven targets on Intel Arc
A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 11
blocked_target_count = 79
failed_target_count = 0
matched_selected_device_bits_count = 11
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 22
host_to_device_bytes = 35200
device_to_host_bytes = 616
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 79
```

## Interpretation

A770-077 continues the layer-3 Q/K/V burn-down by adding `k_proj` after
A770-076 added layer-3 `q_proj`. This is useful correctness evidence, not speed
...
`dispatch_replay_missing` set. The direct continuation is layer-3 `v_proj`
