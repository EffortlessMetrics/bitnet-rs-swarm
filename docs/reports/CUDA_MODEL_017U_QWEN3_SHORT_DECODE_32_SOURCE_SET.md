# CUDA-MODEL-017U Qwen3 Short-Decode 32 Source Set

Status: `short_decode_32` source receipt 3 / 3; aggregate still blocked
...
This report records the third current-source `short_decode_32` strict CUDA
source receipt on the Windows 9950X3D + RTX 5070 Ti lane. It completes the
`short_decode_32` source set only. It does not satisfy the full
CUDA-MODEL-017 aggregate because the `warm_session_3_turns` and
...
```powershell
rtk powershell -Command '$cuda = "C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.9"; $env:CUDA_PATH = $cuda; $env:PATH = "$cuda\bin;" + $env:PATH; $root = "ci\hardware\windows-9950x3d-rtx5070ti\2026-05-23\qwen3-perf-017\run-03"; $model = "C:\bntmp\cuda-model-010\Qwen3-0.6B-Q8_0.gguf"; $base = "ci\hardware\windows-9950x3d-rtx5070ti\2026-05-15"; .\target\debug\bitnet.exe dense-gguf-qwen-short-decode-strict-cuda --model $model --device nvidia-rtx-5070-ti-cuda --device-index 0 --capture-profile qwen3-short-decode-32 --max-new-tokens 32 --all-layer-plan "$base\qwen3-0_6b-cuda-all-layer-plan.json" --model-boundary-fixtures "$base\qwen3-0_6b-model-boundary-fixtures.json" --kv-cache-policy "$base\qwen3-0_6b-kv-cache-policy.json" --sampling-policy "$base\qwen3-0_6b-sampling-policy.json" --one-token-proof "$root\qwen3-0_6b-one-token-cuda.json" --json-out "$root\qwen3-0_6b-short-decode-32-cuda.json"'
```
...
```text
run-03 total_ms=591594.2991 first_token_ms=1062.6149 prefill_ms=509804.0102 decode_total_ms=69475.1594 kernel_time_ms=573592.8994 kernel_launches=12608 H2D_bytes=639446688 D2H_bytes=19447808
```
...
without the CUDA feature; the command was rerun after rebuilding `bitnet-cli`
with `cpu,cuda,full-cli`. The CUDA build produced the existing CUDA deprecation
warnings in `bitnet-kernels`.

The partial aggregate command failed closed as expected with only
`warm_session_3_turns` and `decode_128_from_warm_context` source receipts still
missing.
...
The next source-capture work should start the `warm_session_3_turns` source set
with the same artifact, route, fallback, and claim boundaries.

## Claim Boundary

This report proves only that the current-source Qwen3 `short_decode_32` strict
...
- Any claim that this completed `short_decode_32` set satisfies the remaining
  profiles
