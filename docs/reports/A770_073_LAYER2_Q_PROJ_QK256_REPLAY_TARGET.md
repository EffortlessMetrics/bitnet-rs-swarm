# A770-073 Layer 2 Q Projection QK256 Replay Target

## Scope

A770-073 advances the focused QK256 replay packet by converting the next
remaining layer-2 target, `q_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:2:q_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer2-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer2-q-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer2-q-proj-qk256-replay-target/a770-opencl-qk256-layer2-q-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the A770-068 through
A770-072 overlays, and adds one supplemental focused source for the new layer-2
`q_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer2-q-proj-qk256-replay-target/a770-opencl-qk256-layer2-q-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 7
runnable_target_count = 7
```
...
The selected-device production replay executed all seven targets on Intel Arc
A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 7
blocked_target_count = 83
failed_target_count = 0
matched_selected_device_bits_count = 7
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 14
host_to_device_bytes = 22400
device_to_host_bytes = 392
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 83
```

## Interpretation

A770-073 starts the layer-2 Q/K/V burn-down by adding `q_proj` after the
A770-070 through A770-072 layer-1 Q/K/V replay targets. This is useful
...
`dispatch_replay_missing` set. The direct continuation is layer-2 `k_proj`
