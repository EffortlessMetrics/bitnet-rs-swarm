...
The A770 lane is useful but still diagnostic. A770-130 extends the focused
QK256 replay packet from sixty-three runnable Q/K/V targets to sixty-four: the
layer-0 through layer-20 Q/K/V trios plus layer-21 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-130 covers sixty-four focused Q/K/V rows for one case and one first
...
After A770-130, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-21 `k_proj`, or the next remaining
