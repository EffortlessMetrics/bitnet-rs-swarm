...
The A770 lane is useful but still diagnostic. A770-103 extends the focused
QK256 replay packet from thirty-six runnable Q/K/V targets to thirty-seven:
the layer-0 through layer-11 Q/K/V trios plus layer-12 `q_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-103 covers thirty-seven focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-11 Q/K/V trios plus layer-12 `q_proj`.
...
After A770-103, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-12 `k_proj`, or the next remaining
