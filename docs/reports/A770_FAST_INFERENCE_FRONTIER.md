...
The A770 lane is useful but still diagnostic. A770-146 extends the focused
QK256 replay packet from seventy-nine runnable Q/K/V targets to eighty: the
layer-0 through layer-25 Q/K/V trios plus layer-26 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-146 covers eighty focused Q/K/V rows for one case and one first
...
After A770-146, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-26 `v_proj`, or the next remaining
