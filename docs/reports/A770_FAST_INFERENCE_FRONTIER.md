...
The A770 lane is useful but still diagnostic. A770-136 extends the focused
QK256 replay packet from sixty-nine runnable Q/K/V targets to seventy: the
layer-0 through layer-22 Q/K/V trios plus layer-23 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-136 covers seventy focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-22 Q/K/V trios plus layer-23 `q_proj`.
...
After A770-136, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-23 `k_proj`, or the next remaining
