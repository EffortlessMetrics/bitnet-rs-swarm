...
The A770 lane is useful but still diagnostic. A770-134 extends the focused
QK256 replay packet from sixty-seven runnable Q/K/V targets to sixty-eight: the
layer-0 through layer-21 Q/K/V trios plus layer-22 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-134 covers sixty-eight focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-21 Q/K/V trios plus layer-22 `q_proj` and
...
After A770-134, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-22 `v_proj`, or the next remaining
