...
The A770 lane is useful but still diagnostic. A770-140 extends the focused
QK256 replay packet from seventy-three runnable Q/K/V targets to seventy-four:
the layer-0 through layer-23 Q/K/V trios plus layer-24 `q_proj` and layer-24
`k_proj` for the committed summary-logits first mismatch.
...
1. The current proof is too narrow.
   ...
   A770-140 covers seventy-four focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-23 Q/K/V trios plus layer-24 `q_proj`
   and layer-24 `k_proj`.
   Fast inference needs the same kind of selected-device confidence across the
...
After A770-140, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-24 `v_proj`, or the next remaining
