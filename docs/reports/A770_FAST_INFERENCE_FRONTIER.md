...
The A770 lane is useful but still diagnostic. A770-081 extends the focused
QK256 replay packet from fourteen runnable Q/K/V targets to fifteen: the
layer-0 through layer-4 Q/K/V trios for the committed summary-logits first
mismatch.
...
1. The current proof is too narrow.
   ...
   A770-081 covers fifteen focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-4 Q/K/V trios.
...
After A770-081, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-5 `q_proj`, or the next remaining
