...
The A770 lane is useful but still diagnostic. A770-094 extends the focused
QK256 replay packet from twenty-seven runnable Q/K/V targets to twenty-eight:
the layer-0 through layer-8 Q/K/V trios plus layer-9 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-094 covers twenty-eight focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-8 Q/K/V trios plus layer-9 `q_proj`.
...
After A770-094, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-9 `k_proj`, or the next remaining
