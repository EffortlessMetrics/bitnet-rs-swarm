...
The A770 lane is useful but still diagnostic. A770-109 extends the focused
QK256 replay packet from forty-two runnable Q/K/V targets to forty-three: the
layer-0 through layer-13 Q/K/V trios plus layer-14 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-109 covers forty-three focused Q/K/V rows for one case and one first
...
After A770-109, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-14 `k_proj`, or the next remaining
