...
The A770 lane is useful but still diagnostic. A770-104 extends the focused
QK256 replay packet from thirty-seven runnable Q/K/V targets to thirty-eight:
the layer-0 through layer-11 Q/K/V trios plus layer-12 `q_proj` and `k_proj`
...
1. The current proof is too narrow.
   ...
   A770-104 covers thirty-eight focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-11 Q/K/V trios plus layer-12 `q_proj` and
...
After A770-104, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-12 `v_proj`, or the next remaining
