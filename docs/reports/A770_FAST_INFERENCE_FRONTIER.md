...
The A770 lane is useful but still diagnostic. A770-091 extends the focused
QK256 replay packet from twenty-four runnable Q/K/V targets to twenty-five:
the layer-0 through layer-7 Q/K/V trios plus layer-8 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-091 covers twenty-five focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-7 Q/K/V trios plus layer-8 `q_proj`.
...
After A770-091, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-8 `k_proj`, or the next remaining
