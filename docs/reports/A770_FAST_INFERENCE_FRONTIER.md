...
The A770 lane is useful but still diagnostic. A770-080 extends the focused
QK256 replay packet from thirteen runnable Q/K/V targets to fourteen: the
layer-0 through layer-3 Q/K/V trios plus layer-4 `q_proj` and `k_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-080 covers fourteen focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-3 Q/K/V trios plus layer-4 `q_proj` and
   `k_proj`.
...
After A770-080, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-4 `v_proj`, or the next remaining
