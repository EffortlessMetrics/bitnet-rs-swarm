...
The A770 lane is useful but still diagnostic. A770-122 extends the focused
QK256 replay packet from fifty-five runnable Q/K/V targets to fifty-six: the
layer-0 through layer-17 Q/K/V trios plus layer-18 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-122 covers fifty-six focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-17 Q/K/V trios plus layer-18 `q_proj`
   and `k_proj`.
...
After A770-122, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-18 `v_proj`, or the next remaining
