...
The A770 lane is useful but still diagnostic. A770-085 extends the focused
QK256 replay packet from eighteen runnable Q/K/V targets to nineteen: the
layer-0 through layer-5 Q/K/V trios plus layer-6 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-085 covers nineteen focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-5 Q/K/V trios plus layer-6 `q_proj`.
...
After A770-085, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-6 `k_proj`, or the next remaining
