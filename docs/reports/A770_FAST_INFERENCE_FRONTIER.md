...
The A770 lane is useful but still diagnostic. A770-086 extends the focused
QK256 replay packet from nineteen runnable Q/K/V targets to twenty: the
layer-0 through layer-5 Q/K/V trios plus layer-6 `q_proj` and `k_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-086 covers twenty focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-5 Q/K/V trios plus layer-6 `q_proj` and
...
After A770-086, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-6 `v_proj`, or the next remaining
