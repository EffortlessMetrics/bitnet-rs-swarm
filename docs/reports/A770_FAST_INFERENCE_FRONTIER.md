...
The A770 lane is useful but still diagnostic. A770-071 extends the focused
QK256 replay packet from four runnable Q/K/V targets to five: layer-0 `q_proj`,
layer-0 `k_proj`, layer-0 `v_proj`, layer-1 `q_proj`, and layer-1 `k_proj` for
the committed summary-logits first mismatch. The selected device is Intel Arc
A770 OpenCL and the replay receipts keep
...
1. The current proof is too narrow.
   ...
   A770-071 covers five focused Q/K/V rows for one case and one first mismatch:
   the layer-0 Q/K/V trio plus layer-1 `q_proj` and `k_proj`. Fast inference
   needs the same kind of selected-device confidence across the remaining
   Q/K/V/O projection replay targets, MLP linears, and logits-facing paths that
   can affect generated tokens.
...
After A770-071, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-1 `v_proj`, or the next remaining
