...
The A770 lane is useful but still diagnostic. A770-149 extends the focused
QK256 replay packet from eighty-two runnable Q/K/V targets to eighty-three: the
layer-0 through layer-26 Q/K/V trios plus layer-27 `q_proj` and `k_proj` for
the committed summary-logits first mismatch.
...
1. The current proof is too narrow.
   ...
   A770-149 covers eighty-three focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-26 Q/K/V trios plus layer-27 `q_proj`
   and `k_proj`.
...
After A770-149, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-27 `v_proj`, or the next remaining
