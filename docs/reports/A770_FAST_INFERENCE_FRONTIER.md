...
The A770 lane is useful but still diagnostic. A770-128 extends the focused
QK256 replay packet from sixty-one runnable Q/K/V targets to sixty-two: the
layer-0 through layer-19 Q/K/V trios plus layer-20 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-128 covers sixty-two focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-19 Q/K/V trios plus layer-20 `q_proj`
   and `k_proj`. Fast inference needs the same kind of selected-device
...
After A770-128, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-20 `v_proj`, or the next remaining
