...
The A770 lane is useful but still diagnostic. A770-106 extends the focused
QK256 replay packet from thirty-nine runnable Q/K/V targets to forty: the
layer-0 through layer-12 Q/K/V trios plus layer-13 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-106 covers forty focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-12 Q/K/V trios plus layer-13 `q_proj`.
...
After A770-106, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-13 `k_proj`, or the next remaining
