...
The A770 lane is useful but still diagnostic. A770-152 extends the focused
QK256 replay packet from eighty-five runnable Q/K/V targets to eighty-six: the
layer-0 through layer-27 Q/K/V trios plus layer-28 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-152 covers eighty-six focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-27 Q/K/V trios plus layer-28 `q_proj`
   and `k_proj`.
...
After A770-152, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-28 `v_proj`, or the next remaining
