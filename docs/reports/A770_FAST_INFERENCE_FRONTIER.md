...
The A770 lane is useful but still diagnostic. A770-100 extends the focused
QK256 replay packet from thirty-three runnable Q/K/V targets to thirty-four:
the layer-0 through layer-10 Q/K/V trios plus layer-11 `q_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-100 covers thirty-four focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-10 Q/K/V trios plus layer-11 `q_proj`.
...
After A770-100, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-11 `k_proj`, or the next remaining
