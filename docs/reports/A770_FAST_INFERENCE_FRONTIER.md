...
The A770 lane is useful but still diagnostic. A770-076 extends the focused
QK256 replay packet from nine runnable Q/K/V targets to ten: the layer-0,
layer-1, and layer-2 Q/K/V trios plus layer-3 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-076 covers ten focused Q/K/V rows for one case and one first mismatch:
   the layer-0, layer-1, and layer-2 Q/K/V trios plus layer-3 `q_proj`. Fast
...
After A770-076, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-3 `k_proj`, or the next remaining
