...
The A770 lane is useful but still diagnostic. A770-092 extends the focused
QK256 replay packet from twenty-five runnable Q/K/V targets to twenty-six:
the layer-0 through layer-7 Q/K/V trios plus layer-8 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-092 covers twenty-six focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-7 Q/K/V trios plus layer-8 `q_proj` and
   `k_proj`.
...
After A770-092, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-8 `v_proj`, or the next remaining
