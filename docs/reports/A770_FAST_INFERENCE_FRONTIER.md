...
The A770 lane is useful but still diagnostic. A770-145 extends the focused
QK256 replay packet from seventy-eight runnable Q/K/V targets to seventy-nine:
the layer-0 through layer-25 Q/K/V trios plus layer-26 `q_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-145 covers seventy-nine focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-25 Q/K/V trios plus layer-26 `q_proj`.
...
After A770-145, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-26 `k_proj`, or the next remaining
