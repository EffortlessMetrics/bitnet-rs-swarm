...
The A770 lane is useful but still diagnostic. A770-142 extends the focused
QK256 replay packet from seventy-five runnable Q/K/V targets to seventy-six:
the layer-0 through layer-24 Q/K/V trios plus layer-25 `q_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-142 covers seventy-six focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-24 Q/K/V trios plus layer-25 `q_proj`.
...
After A770-142, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-25 `k_proj`, or the next remaining
