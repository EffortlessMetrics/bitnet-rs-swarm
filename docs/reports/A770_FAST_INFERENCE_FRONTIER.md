...
The A770 lane is useful but still diagnostic. A770-098 extends the focused
QK256 replay packet from thirty-one runnable Q/K/V targets to thirty-two: the
layer-0 through layer-9 Q/K/V trios plus layer-10 `q_proj` and `k_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-098 covers thirty-two focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-9 Q/K/V trios plus layer-10 `q_proj`
   and `k_proj`.
...
After A770-098, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-10 `v_proj`, or the next remaining
