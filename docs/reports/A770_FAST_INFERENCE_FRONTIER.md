...
The A770 lane is useful but still diagnostic. A770-113 extends the focused
QK256 replay packet from forty-six runnable Q/K/V targets to forty-seven: the
layer-0 through layer-14 Q/K/V trios plus layer-15 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-113 covers forty-seven focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-14 Q/K/V trios plus layer-15 `q_proj`
   and `k_proj`.
...
After A770-113, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-15 `v_proj`, or the next remaining
