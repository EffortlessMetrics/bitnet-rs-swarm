...
The A770 lane is useful but still diagnostic. A770-095 extends the focused
QK256 replay packet from twenty-eight runnable Q/K/V targets to twenty-nine:
the layer-0 through layer-8 Q/K/V trios plus layer-9 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-095 covers twenty-nine focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-8 Q/K/V trios plus layer-9 `q_proj` and
   `k_proj`.
...
After A770-095, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-9 `v_proj`, or the next remaining
