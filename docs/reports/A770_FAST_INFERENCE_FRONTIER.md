...
The A770 lane is useful but still diagnostic. A770-139 extends the focused
QK256 replay packet from seventy-two runnable Q/K/V targets to seventy-three:
the layer-0 through layer-23 Q/K/V trios plus layer-24 `q_proj` for the
committed summary-logits first mismatch.
...
1. The current proof is too narrow.
   ...
   A770-139 covers seventy-three focused Q/K/V rows for one case and one first
...
After A770-139, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-24 `k_proj`, or the next remaining
