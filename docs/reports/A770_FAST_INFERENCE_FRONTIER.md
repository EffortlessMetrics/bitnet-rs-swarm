...
The A770 lane is useful but still diagnostic. A770-116 extends the focused
QK256 replay packet from forty-nine runnable Q/K/V targets to fifty: the
layer-0 through layer-15 Q/K/V trios plus layer-16 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-116 covers fifty focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-15 Q/K/V trios plus layer-16 `q_proj`
   and `k_proj`.
...
After A770-116, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-16 `v_proj`, or the next remaining
