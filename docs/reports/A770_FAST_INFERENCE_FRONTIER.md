...
The A770 lane is useful but still diagnostic. A770-089 extends the focused
QK256 replay packet from twenty-two runnable Q/K/V targets to twenty-three: the
layer-0 through layer-6 Q/K/V trios plus layer-7 `q_proj` and `k_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-089 covers twenty-three focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-6 Q/K/V trios plus layer-7 `q_proj` and
   `k_proj`.
...
After A770-089, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-7 `v_proj`, or the next remaining
