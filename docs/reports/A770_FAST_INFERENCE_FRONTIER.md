...
The A770 lane is useful but still diagnostic. A770-137 extends the focused
QK256 replay packet from seventy runnable Q/K/V targets to seventy-one: the
layer-0 through layer-22 Q/K/V trios plus layer-23 `q_proj` and layer-23
...
1. The current proof is too narrow.
   ...
   A770-137 covers seventy-one focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-22 Q/K/V trios plus layer-23 `q_proj`
   and layer-23 `k_proj`. Fast inference needs the same kind of selected-device
...
After A770-137, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-23 `v_proj`, or the next remaining
