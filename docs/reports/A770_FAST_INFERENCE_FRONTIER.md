...
The A770 lane is useful but still diagnostic. A770-124 extends the focused
QK256 replay packet from fifty-seven runnable Q/K/V targets to fifty-eight:
the layer-0 through layer-18 Q/K/V trios plus layer-19 `q_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-124 covers fifty-eight focused Q/K/V rows for one case and one first
...
After A770-124, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-19 `k_proj`, or the next remaining
