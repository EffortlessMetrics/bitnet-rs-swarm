...
The A770 lane is useful but still diagnostic. A770-090 extends the focused
QK256 replay packet from twenty-three runnable Q/K/V targets to twenty-four:
the layer-0 through layer-7 Q/K/V trios for the committed summary-logits first
mismatch.
...
1. The current proof is too narrow.
   ...
   A770-090 covers twenty-four focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-7 Q/K/V trios.
...
After A770-090, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-8 `q_proj`, or the next remaining
