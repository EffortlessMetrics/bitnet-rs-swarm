...
The A770 lane is useful but still diagnostic. A770-131 extends the focused
QK256 replay packet from sixty-four runnable Q/K/V targets to sixty-five: the
layer-0 through layer-20 Q/K/V trios plus layer-21 `q_proj` and layer-21
...
1. The current proof is too narrow.
   ...
   A770-131 covers sixty-five focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-20 Q/K/V trios plus layer-21 `q_proj`
   and `k_proj`.
...
After A770-131, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-21 `v_proj`, or the next remaining
