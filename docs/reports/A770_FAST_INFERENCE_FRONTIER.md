...
The A770 lane is useful but still diagnostic. A770-107 extends the focused
QK256 replay packet from forty runnable Q/K/V targets to forty-one: the layer-0
through layer-12 Q/K/V trios plus layer-13 `q_proj` and `k_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-107 covers forty-one focused Q/K/V rows for one case and one first
...
After A770-107, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-13 `v_proj`, or the next remaining
