...
The A770 lane is useful but still diagnostic. A770-110 extends the focused
QK256 replay packet from forty-three runnable Q/K/V targets to forty-four: the
layer-0 through layer-13 Q/K/V trios plus layer-14 `q_proj` and `k_proj` for
...
1. The current proof is too narrow.
   ...
   A770-110 covers forty-four focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-13 Q/K/V trios plus layer-14 `q_proj`
   and `k_proj`.
...
After A770-110, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-14 `v_proj`, or the next remaining
