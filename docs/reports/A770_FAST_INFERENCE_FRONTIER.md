...
The A770 lane is useful but still diagnostic. A770-154 extends the focused
QK256 replay packet from eighty-seven runnable Q/K/V targets to eighty-eight:
the layer-0 through layer-28 Q/K/V trios plus layer-29 `q_proj` for the
...
1. The current proof is too narrow.
   ...
   A770-154 covers eighty-eight focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-28 Q/K/V trios plus layer-29 `q_proj`.
...
After A770-154, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-29 `k_proj`, or the next remaining
