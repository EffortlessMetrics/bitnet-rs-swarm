...
The A770 lane is useful but still diagnostic. A770-088 extends the focused
QK256 replay packet from twenty-one runnable Q/K/V targets to twenty-two: the
layer-0 through layer-6 Q/K/V trios plus layer-7 `q_proj` for the committed
...
1. The current proof is too narrow.
   ...
   A770-088 covers twenty-two focused Q/K/V rows for one case and one first
   mismatch: the layer-0 through layer-6 Q/K/V trios plus layer-7 `q_proj`.
...
After A770-088, the next honest step is one more focused replay target, not a
speed PR: capture and replay layer-7 `k_proj`, or the next remaining
