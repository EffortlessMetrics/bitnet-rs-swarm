# A770-134 Layer 22 K Projection QK256 Replay Target

## Scope

A770-134 advances the focused QK256 replay packet by converting one more
remaining target, layer-22 `k_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:22:k_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer22-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer22-k-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer22-k-proj-qk256-replay-target/a770-opencl-qk256-layer22-k-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, records the new layer-22
`k_proj` source as the primary focused source, and supplements it with the
previous A770-133 focused source list.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer22-k-proj-qk256-replay-target/a770-opencl-qk256-layer22-k-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 68
runnable_target_count = 68
```

The runnable targets are the layer-0 through layer-21 Q/K/V trios plus
layer-22 `q_proj` and `k_proj`.

The selected-device production replay executed all sixty-eight runnable targets
on Intel Arc A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 68
blocked_target_count = 22
failed_target_count = 0
matched_selected_device_bits_count = 68
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 136
host_to_device_bytes = 217600
device_to_host_bytes = 3808
```
...
target_id = a770_summary_seed770024_keywords_014:step9:22:k_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
focused_device_output_bits = 1057758772
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 22
first_blocked_target = a770_summary_seed770024_keywords_014:step9:22:v_proj
first_blocked_reason = dispatch_replay_missing
```

## Interpretation

A770-134 continues the layer-22 Q/K/V trio under the same selected-device,
...
`dispatch_replay_missing` set. The direct continuation is layer-22 `v_proj`
...
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer22-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer22-k-proj-raw-operands.json --per-prompt-timeout-seconds 300
target/debug/a770-opencl-production-replay-instrumentation.exe --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer22-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer22-k-proj-raw-operands.json --focused-source ... --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-134 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer22-k-proj-qk256-replay-target/a770-opencl-qk256-layer22-k-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer22-k-proj-qk256-replay-target/a770-opencl-qk256-layer22-k-proj-target-replay.json
PowerShell receipt assertion for target_count=90, dispatch_replay_target_count=68, runnable_target_count=68, executed_target_count=68, blocked_target_count=22, failed_target_count=0, matched_selected_device_bits_count=68, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-134, and layer-22 k_proj executed fallback-free with selected-device bit matches
...
BITNET_QKV_PROJECTION_DISPATCH_REPLAY_LAYER=22
...
The active goal's `cargo run --locked -p xtask --no-default-features -- campaign
check intel-a770` and `cargo run --locked -p xtask --no-default-features --
campaign generate --check` wrappers did not produce output before the local
Windows command timeout. WSL has no `cargo` binary installed, so the substitute
