# A770-091 Layer 8 Q Projection QK256 Replay Target

## Scope

A770-091 advances the focused QK256 replay packet by converting one more
remaining target, layer-8 `q_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:8:q_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer8-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer8-q-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer8-q-proj-qk256-replay-target/a770-opencl-qk256-layer8-q-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the A770-068 through
A770-090 overlays, and adds one supplemental focused source for the new layer-8
`q_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer8-q-proj-qk256-replay-target/a770-opencl-qk256-layer8-q-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 25
runnable_target_count = 25
```

The runnable targets are the layer-0 through layer-7 Q/K/V trios plus layer-8
`q_proj`.

The selected-device production replay executed all twenty-five targets on Intel
Arc A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 25
blocked_target_count = 65
failed_target_count = 0
matched_selected_device_bits_count = 25
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 50
host_to_device_bytes = 80000
device_to_host_bytes = 1400
```
...
target_id = a770_summary_seed770024_keywords_014:step9:8:q_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
```text
dispatch_replay_missing = 65
```

## Interpretation

A770-091 starts the layer-8 focused replay trio by adding `q_proj` after
A770-090 completed layer 7. This is useful correctness evidence, not speed
...
`dispatch_replay_missing` set. The direct continuation is layer-8 `k_proj`
...
```text
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer8-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer8-q-proj-raw-operands.json --per-prompt-timeout-seconds 300
target/debug/a770-opencl-production-replay-instrumentation.exe --focused-source ... --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer8-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer8-q-proj-raw-operands.json --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-091 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer8-q-proj-qk256-replay-target/a770-opencl-qk256-layer8-q-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer8-q-proj-qk256-replay-target/a770-opencl-qk256-layer8-q-proj-target-replay.json
pwsh receipt assertion for target_count=90, dispatch_replay_target_count=25, runnable_target_count=25, executed_target_count=25, blocked_target_count=65, failed_target_count=0, matched_selected_device_bits_count=25, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-091, and layer-8 q_proj executed fallback-free with selected-device bit match
```
