# A770-074 Layer 2 K Projection QK256 Replay Target

## Scope

A770-074 advances the focused QK256 replay packet by converting the next
remaining layer-2 target, `k_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:2:k_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer2-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer2-k-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer2-k-proj-qk256-replay-target/a770-opencl-qk256-layer2-k-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the A770-068 through
A770-073 overlays, and adds one supplemental focused source for the new layer-2
`k_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer2-k-proj-qk256-replay-target/a770-opencl-qk256-layer2-k-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 8
runnable_target_count = 8
```
...
The selected-device production replay executed all eight targets on Intel Arc
A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 8
blocked_target_count = 82
failed_target_count = 0
matched_selected_device_bits_count = 8
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 16
host_to_device_bytes = 25600
device_to_host_bytes = 448
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 82
```

## Interpretation

A770-074 continues the layer-2 Q/K/V burn-down by adding `k_proj` after the
A770-073 layer-2 `q_proj` replay target. This is useful correctness evidence,
...
`dispatch_replay_missing` set. The direct continuation is layer-2 `v_proj`
