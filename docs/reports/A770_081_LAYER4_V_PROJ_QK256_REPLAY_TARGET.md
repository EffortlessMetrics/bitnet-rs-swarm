# A770-081 Layer 4 V Projection QK256 Replay Target

## Scope

A770-081 advances the focused QK256 replay packet by converting the next
remaining target, layer-4 `v_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:4:v_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer4-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer4-v-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer4-v-proj-qk256-replay-target/a770-opencl-qk256-layer4-v-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the A770-068 through
A770-080 overlays, and adds one supplemental focused source for the new layer-4
`v_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer4-v-proj-qk256-replay-target/a770-opencl-qk256-layer4-v-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 15
runnable_target_count = 15
```
...
The selected-device production replay executed all fifteen targets on Intel Arc
A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 15
blocked_target_count = 75
failed_target_count = 0
matched_selected_device_bits_count = 15
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 30
host_to_device_bytes = 48000
device_to_host_bytes = 840
```
...
target_id = a770_summary_seed770024_keywords_014:step9:4:v_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
```text
dispatch_replay_missing = 75
```

## Interpretation

A770-081 completes the layer-4 Q/K/V focused replay trio by adding `v_proj`
after A770-079 and A770-080 added layer-4 `q_proj` and `k_proj`. This is useful
...
`dispatch_replay_missing` set. The direct continuation is layer-5 `q_proj`
...
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer4-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer4-v-proj-raw-operands.json --per-prompt-timeout-seconds 300
cargo run --locked -p bitnet-kernels --no-default-features --features opencl --bin a770-opencl-production-replay-instrumentation -- --focused-source ... --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer4-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer4-v-proj-raw-operands.json --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-081 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer4-v-proj-qk256-replay-target/a770-opencl-qk256-layer4-v-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-02/a770-layer4-v-proj-qk256-replay-target/a770-opencl-qk256-layer4-v-proj-target-replay.json
pwsh receipt assertion for target_count=90, dispatch_replay_target_count=15, runnable_target_count=15, executed_target_count=15, blocked_target_count=75, failed_target_count=0, matched_selected_device_bits_count=15, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-081, and layer-4 v_proj executed fallback-free with selected-device bit match
```
