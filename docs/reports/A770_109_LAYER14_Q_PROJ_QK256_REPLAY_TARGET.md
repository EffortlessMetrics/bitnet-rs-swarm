# A770-109 Layer 14 Q Projection QK256 Replay Target

## Scope

A770-109 advances the focused QK256 replay packet by converting one more
remaining target, layer-14 `q_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:14:q_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer14-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer14-q-proj-raw-operands.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer14-q-proj-qk256-replay-target/a770-opencl-qk256-layer14-q-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, keeps the previous
forty-two focused replay sources, and adds one supplemental focused source for
the new layer-14 `q_proj` target.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer14-q-proj-qk256-replay-target/a770-opencl-qk256-layer14-q-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 43
runnable_target_count = 43
```

The runnable targets are the layer-0 through layer-13 Q/K/V trios plus
layer-14 `q_proj`.

The selected-device production replay executed all forty-three targets on Intel
Arc A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 43
blocked_target_count = 47
failed_target_count = 0
matched_selected_device_bits_count = 43
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 86
host_to_device_bytes = 137600
device_to_host_bytes = 2408
```
...
target_id = a770_summary_seed770024_keywords_014:step9:14:q_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
selected_device_output_bits = 3189777106
```

The remaining blockers are still ledgered:

```text
dispatch_replay_missing = 47
first_blocked_target = a770_summary_seed770024_keywords_014:step9:14:k_proj
first_blocked_reason = dispatch_replay_missing
```

## Interpretation

A770-109 starts the layer-14 focused replay Q/K/V trio by adding `q_proj` after
A770-108 completed the layer-13 trio. This is useful correctness evidence, not
...
`dispatch_replay_missing` set. The direct continuation is layer-14 `k_proj`
...
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer14-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer14-q-proj-raw-operands.json --per-prompt-timeout-seconds 300
target/debug/a770-opencl-production-replay-instrumentation.exe --focused-source ... --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer14-q-proj-qk256-replay-target/a770-opencl-summary-logits-layer14-q-proj-raw-operands.json --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-109 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer14-q-proj-qk256-replay-target/a770-opencl-qk256-layer14-q-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-03/a770-layer14-q-proj-qk256-replay-target/a770-opencl-qk256-layer14-q-proj-target-replay.json
pwsh receipt assertion for target_count=90, dispatch_replay_target_count=43, runnable_target_count=43, executed_target_count=43, blocked_target_count=47, failed_target_count=0, matched_selected_device_bits_count=43, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-109, and layer-14 q_proj executed fallback-free with selected-device bit matches
```
...
BITNET_QKV_PROJECTION_DISPATCH_REPLAY_LAYER=14
