# A770-159 Full Projection Operands Capture Boundary

## Scope

A770-159 uses the committed A770-158 projection replay hook receipt to inspect
the linked full projection operand source for the same narrow surface:
...
source_packet = A770-158 projection replay hook boundary receipt
```
...
The A770-159 manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-05/a770-full-projection-operands-capture-boundary/a770-opencl-qkv-full-projection-operands-capture-manifest.json
```

The A770-159 receipt is:
...
The receipt preserves the A770-158 replay hook and narrows the remaining
operand blocker:
...
Each target has a committed source packet, but each packet contains only
single-output-row focused packed QK256 bytes:
...
A770-159 does not execute projection-level replay because the committed operand
sources do not contain full projection packed weight rows. This is a blocker
ledger, not a failed replay. The selected-device route remains Intel Arc A770
OpenCL with `fallback_used=false`, and no CPU fallback or generic OpenCL result
is substituted.

The next runtime step is to add a capture source that records full projection
Q/K/V packed rows for this same surface, then run the existing bounded
projection replay hook fallback-free.
...
cargo run --locked -p bitnet-kernels --no-default-features --features opencl --bin a770-opencl-production-replay-instrumentation -- --projection-source ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-projection-replay-hook-boundary/a770-opencl-qkv-projection-replay-hook.json --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --projection-layer 0 --work-item A770-159 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-05/a770-full-projection-operands-capture-boundary/a770-opencl-qkv-full-projection-operands-capture-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-05/a770-full-projection-operands-capture-boundary/a770-opencl-qkv-full-projection-operands-capture.json
git diff --check
parse the A770-159 manifest and receipt with ConvertFrom-Json
assert selected_backend=intel-arc-a770-opencl, runtime_api=opencl, fallback_used=false, projection_replay_kernel_executed=false, and projection_level_full_projection_weight_rows_missing is present
...
intel-a770` remained in `sentencepiece-sys` compilation for 20 minutes and was
stopped. Earlier attempts to run campaign generation also timed out while
building `xtask`; no generated campaign dashboards are changed by this PR.
