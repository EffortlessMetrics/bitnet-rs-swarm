# A770-138 Layer 23 V Projection QK256 Replay Target

## Scope

A770-138 advances the focused QK256 replay packet by converting one more
remaining target, layer-23 `v_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:23:v_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer23-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer23-v-proj-raw-operands.json
```

The focused answer-corpus run receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer23-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer23-v-proj-raw-operands-runs/a770_summary_seed770024_keywords_014.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer23-v-proj-qk256-replay-target/a770-opencl-qk256-layer23-v-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, records the new layer-23
`v_proj` source as the primary focused source, and supplements it with the
previous A770-137 focused source list.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer23-v-proj-qk256-replay-target/a770-opencl-qk256-layer23-v-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 72
runnable_target_count = 72
```

The runnable targets are the layer-0 through layer-23 Q/K/V trios.

The selected-device production replay executed all seventy-two runnable targets
on Intel Arc A770 OpenCL with no fallback and matched the selected-device bits:

```text
executed_target_count = 72
blocked_target_count = 18
failed_target_count = 0
matched_selected_device_bits_count = 72
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 144
host_to_device_bytes = 230400
device_to_host_bytes = 4032
```
...
target_id = a770_summary_seed770024_keywords_014:step9:23:v_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
focused_device_output_bits = 1069296635
...
```text
dispatch_replay_missing = 18
first_blocked_target = a770_summary_seed770024_keywords_014:step9:24:q_proj
first_blocked_reason = dispatch_replay_missing
```

## Interpretation

A770-138 completes the layer-23 Q/K/V trio under the same selected-device,
...
`dispatch_replay_missing` set. The direct continuation is layer-24 `q_proj`
...
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer23-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer23-v-proj-raw-operands.json --per-prompt-timeout-seconds 300
target/debug/a770-opencl-production-replay-instrumentation.exe --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer23-v-proj-qk256-replay-target/a770-opencl-summary-logits-layer23-v-proj-raw-operands.json --focused-source ... --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-138 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer23-v-proj-qk256-replay-target/a770-opencl-qk256-layer23-v-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer23-v-proj-qk256-replay-target/a770-opencl-qk256-layer23-v-proj-target-replay.json
PowerShell receipt assertion for target_count=90, dispatch_replay_target_count=72, runnable_target_count=72, executed_target_count=72, blocked_target_count=18, failed_target_count=0, matched_selected_device_bits_count=72, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-138, and layer-23 v_proj executed fallback-free with selected-device bit matches
...
BITNET_QKV_PROJECTION_DISPATCH_REPLAY_LAYER=23
