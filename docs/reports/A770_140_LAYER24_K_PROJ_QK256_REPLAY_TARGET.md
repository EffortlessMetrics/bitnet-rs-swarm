# A770-140 Layer 24 K Projection QK256 Replay Target

## Scope

A770-140 advances the focused QK256 replay packet by converting one more
remaining target, layer-24 `k_proj`, into selected-device Intel Arc A770 OpenCL
production replay.
...
new_target = a770_summary_seed770024_keywords_014:step9:24:k_proj
...
```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer24-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer24-k-proj-raw-operands.json
```

The focused answer-corpus run receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer24-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer24-k-proj-raw-operands-runs/a770_summary_seed770024_keywords_014.json
```

The combined manifest is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer24-k-proj-qk256-replay-target/a770-opencl-qk256-layer24-k-proj-target-replay-manifest.json
```

It preserves the 90-target focused manifest shape, records the new layer-24
`k_proj` source as the primary focused source, and supplements it with the
previous A770-139 focused source list.

The selected-device A770 OpenCL replay receipt is:

```text
ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer24-k-proj-qk256-replay-target/a770-opencl-qk256-layer24-k-proj-target-replay.json
```
...
```text
target_count = 90
dispatch_replay_target_count = 74
runnable_target_count = 74
```

The runnable targets are the layer-0 through layer-23 Q/K/V trios plus
layer-24 `q_proj` and layer-24 `k_proj`.

The selected-device production replay executed all seventy-four runnable
targets on Intel Arc A770 OpenCL with no fallback and matched the
selected-device bits:

```text
executed_target_count = 74
blocked_target_count = 16
failed_target_count = 0
matched_selected_device_bits_count = 74
all_executed_targets_match_selected_device_bits = true
kernel_invocations = 148
host_to_device_bytes = 236800
device_to_host_bytes = 4144
```
...
target_id = a770_summary_seed770024_keywords_014:step9:24:k_proj
selected_backend = intel-arc-a770-opencl
runtime_api = opencl
fallback_used = false
...
focused_device_output_bits = 3206687323
...
```text
dispatch_replay_missing = 16
first_blocked_target = a770_summary_seed770024_keywords_014:step9:24:v_proj
first_blocked_reason = dispatch_replay_missing
```

## Interpretation

A770-140 extends the layer-24 Q/K/V trio under the same selected-device,
...
`dispatch_replay_missing` set. The direct continuation is layer-24 `v_proj`
...
target/release/bitnet.exe answer-corpus --corpus ci/quality/a770-bitnet-answer-readiness-corpus.yaml --model E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/ggml-model-i2_s.gguf --tokenizer E:/Code/Rust/BitNet-rs/models/BitNet-b1.58-2B-4T/tokenizer.json --device intel-a770-opencl --case-id a770_summary_seed770024_keywords_014 --dump-logit-steps 24 --logits-topk 20 --json-out ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer24-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer24-k-proj-raw-operands.json --per-prompt-timeout-seconds 300
target/debug/a770-opencl-production-replay-instrumentation.exe --focused-source ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer24-k-proj-qk256-replay-target/a770-opencl-summary-logits-layer24-k-proj-raw-operands.json --focused-source ... --case-id a770_summary_seed770024_keywords_014 --first-mismatch-index 9 --work-item A770-140 --manifest ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer24-k-proj-qk256-replay-target/a770-opencl-qk256-layer24-k-proj-target-replay-manifest.json --receipt ci/hardware/amd-5700x-intel-a770/2026-06-04/a770-layer24-k-proj-qk256-replay-target/a770-opencl-qk256-layer24-k-proj-target-replay.json
PowerShell receipt assertion for target_count=90, dispatch_replay_target_count=74, runnable_target_count=74, executed_target_count=74, blocked_target_count=16, failed_target_count=0, matched_selected_device_bits_count=74, all_executed_targets_match_selected_device_bits=true, fallback_used=false, work_item=A770-140, and layer-24 k_proj executed fallback-free with selected-device bit matches
PowerShell JSON parse for all four new receipt files
npx markdownlint-cli2 docs/reports/A770_140_LAYER24_K_PROJ_QK256_REPLAY_TARGET.md docs/reports/A770_FAST_INFERENCE_FRONTIER.md docs/tracking/campaigns/intel-a770/CAMPAIGN.md
git diff --check
...
BITNET_QKV_PROJECTION_DISPATCH_REPLAY_LAYER=24
...
The active goal's exact Windows wrapper command:

```text
cargo run --locked -p xtask --no-default-features -- campaign check intel-a770
```

timed out before producing output after roughly 248 seconds. The substitute
evidence used the already-built repo xtask binary under WSL. Both substitute
xtask commands passed, and `campaign check intel-a770` reported only the known
A770-045 event-note warnings.

A raw markdownlint run that included generated dashboard files reported MD060
table-column-style errors in generated campaign tables. The hand-authored
report, frontier, and campaign markdown passed markdownlint, and the generated
dashboard files were validated with `campaign generate --check`.
