# CUDA-MODEL-017T Qwen3 Short-Decode 32 Source Receipt

Status: `short_decode_32` source receipt 2 / 3; aggregate still blocked
...
generated. This report records the second strict CUDA `short_decode_32` source
...
```text
run-02 total_ms=483398.9859 first_token_ms=1606.822 prefill_ms=436201.1385 decode_total_ms=35108.1469 kernel_time_ms=467594.8359 kernel_launches=12608 H2D_bytes=639446688 D2H_bytes=19447808
```
...
Result: passed where success is expected. The first capture attempt failed
closed before writing a receipt because the local debug CLI had been rebuilt
without the CUDA feature; the command was rerun after rebuilding
`bitnet-cli` with `cpu,cuda,full-cli`. The CUDA build produced the existing
CUDA deprecation warnings in `bitnet-kernels`.

The partial aggregate command failed closed as expected because
`warm_session_3_turns` and `decode_128_from_warm_context` source receipts are
still absent. The committed tracker remains stricter than the current missing
input error and still requires the third `short_decode_32` source receipt before
the profile set is complete.

`rtk git diff --check`, `rtk cargo test --locked -p bitnet-bench-receipts
--no-default-features qwen3`, and `rtk cargo test --locked -p bitnet-cli
--no-default-features --features cpu,full-cli receipts_explain` passed locally.
`rtk cargo run --locked -p xtask --no-default-features -- campaign check
nvidia-5070ti` exceeded the local 10-minute validation window. `rtk cargo run
--locked -p xtask --no-default-features -- campaign generate --check` returned
non-zero during local build after compiling `sentencepiece-sys`, and a retry
with `rtk err` exceeded the same 10-minute local window. Stale local campaign
cargo processes from those attempts were stopped; remote Campaign Tracker /
Doctor remains required for the PR.
...
The next source-capture work should collect `short_decode_32` run 03 with the
same artifact, route, fallback, and claim boundaries.

## Claim Boundary

This report proves only that two current-source Qwen3 `short_decode_32` strict
CUDA receipts exist for CUDA-MODEL-017.
